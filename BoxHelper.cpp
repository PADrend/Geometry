...
std::vector<Box_f> splitUpBox(const Box_f & box,
							  ...
							  unsigned int partsZ) {
	if(box.isInvalid()){
		std::cerr << __FILE__ << ":" << __LINE__ << ":" << std::endl << "\t you are splitting invalid boxes" << std::endl;
	}
	...
	for (unsigned int  cz = 1; cz <= partsZ; ++cz)
	{
		...
		for (unsigned int  cy = 1; cy <= partsY; ++cy)
		{
			...
			for (unsigned int  cx = 1; cx <= partsX; ++cx)
			{
				...
				{
					const float uX = static_cast<float> (partsX - cx)
									 / static_cast<float> (partsX);
					const float vX = static_cast<float> (cx)
									 / static_cast<float> (partsX);
				...
				{
					const float uY = static_cast<float> (partsY - cy)
									 / static_cast<float> (partsY);
					const float vY = static_cast<float> (cy)
									 / static_cast<float> (partsY);
				...
				{
					const float uZ = static_cast<float> (partsZ - cz)
									 / static_cast<float> (partsZ);
					const float vZ = static_cast<float> (cz)
									 / static_cast<float> (partsZ);
