...
class _Vec4 {
	...
	//@{
		...
		_Vec4() 							{	setValue(static_cast<value_t>(0));	}
		...
		_Vec4(value_t _x, value_t _y, value_t _z, value_t _w) 	{	setValue(_x,_y,_z,_w);	}

		/*!	[ctor]
			@param v used to initialize the new Vec4
			@note values are converted by casting	*/
		template<class other_t>
		explicit _Vec4(const _Vec4<other_t> & v) 		{	setValue(static_cast<value_t>(v.x()), static_cast<value_t>(v.y()), static_cast<value_t>(v.z()), static_cast<value_t>(v.w()));	}
		...
		explicit _Vec4(const value_t * v) 			{	setValue(v[0],v[1],v[2],v[3]);	}
	...
	//@{
		...
		value_t& operator[](uint_fast8_t nr) 				{	return vec[nr];	}

		/*! const array-access operator	*/
		const value_t& operator[](uint_fast8_t nr) const {	return vec[nr];	}

		/*! @return the underlying array of this	*/
		const value_t * getVec() const 						{	return vec;	}

		/*! @return the i-th component of this	*/
		const value_t get(const int & i) const 				{	return vec[i];	}

		/*! @return the x component of this
			@note equal to this[0]	*/
		const value_t getX() const 							{	return vec[0];	}
		const value_t x() const 							{	return vec[0];	}

		/*! @return the y component of this
			@note equal to this[1]	*/
		const value_t getY() const 							{	return vec[1];	}
		const value_t y() const 							{	return vec[1];	}

		/*! @return the z component of this
			@note equal to this[2]	*/
		const value_t getZ() const 							{	return vec[2];	}
		const value_t z() const 							{	return vec[2];	}
	
		/*! @return the z component of this
			@note equal to this[3]	*/
		const value_t getW() const 							{	return vec[3];	}
		const value_t w() const 							{	return vec[3];	}
		...
		inline value_t lengthSquared() const 				{	return dot(*this);	}
	...
	//@{
		...
		void setValue(const value_t t) 	{	setValue(t,t,t,t);	}

		/*! sets the components of this
			@param v the first three values of v are used to set the components of this	*/
		void setValue(const value_t *v) 	{	setValue(v[0],v[1],v[2],v[3]);	}

		/*! sets the x component
			@param x the value the x component of this is set to
			@note equal to this[0]=x	*/
		void setX(const value_t _x)  		{	vec[0]=_x;	}
		void x(const value_t v) 			{	vec[0]=v;	}

		/*! sets the y component
			@param y the value the y component of this is set to
			@note equal to this[1]=y	*/
		void setY(const value_t _y)  		{	vec[1]=_y;	}
		void y(const value_t v) 			{	vec[1]=v;	}

		/*! sets the z component
			@param z the value the z component of this is set to
			@note equal to this[2]=z	*/
		void setZ(const value_t _z) 		{	vec[2]=_z;	}
		void z(const value_t v) 			{	vec[2]=v;	}

		/*! sets the z component
			@param z the value the z component of this is set to
			@note equal to this[2]=z	*/
		void setW(const value_t _w) 		{	vec[3]=_w;	}
		void w(const value_t v) 			{	vec[3]=v;	}
	...
	//@{
		...
		/*! @return a new Vec4 representing the negation of this
			@note this remains unchanged	*/
		const _Vec4 operator-() const					{	return _Vec4( -vec[0], -vec[1], -vec[2], -vec[3] );	}
	...
	//@{
		...
		inline bool equals(const vec3_t &other, value_t epsilon) const {
			return std::abs(other.vec[0] - vec[0]) <= epsilon
				&& std::abs(other.vec[1] - vec[1]) <= epsilon
				&& std::abs(other.vec[2] - vec[2]) <= epsilon
				&& std::abs(other.vec[3] - vec[3]) <= epsilon;
		}
