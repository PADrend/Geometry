...
class _Matrix3x3  {
	...
	public:
		...
		explicit _Matrix3x3(	value_t r0c0, value_t r0c1, value_t r0c2, 
								value_t r1c0, value_t r1c1, value_t r1c2, 
	...
	//@{
		...
		inline bool isIdentity()const{
			return 	at(R0C0)==1 && at(R0C1)==0 && at(R0C2)==0 &&
					at(R1C0)==0 && at(R1C1)==1 && at(R1C2)==0 &&
	...
	//@{
		...
		void set( value_t r0c0, value_t r0c1, value_t r0c2, 
				  value_t r1c0, value_t r1c1, value_t r1c2, 
				  value_t r2c0, value_t r2c1, value_t r2c2) {
			set(R0C0,r0c0);		set(R0C1,r0c1);		set(R0C2,r0c2);	
			set(R1C0,r1c0);		set(R1C1,r1c1);		set(R1C2,r1c2);	
			set(R2C0,r2c0);		set(R2C1,r2c1);		set(R2C2,r2c2);	
		}
		...
		static _Matrix3x3 createRotation(const angle_t & angle, const vec3_t & axis) {
			...
			return _Matrix3x3(
				t * xx + c,					t * xy - s * axis.getZ(),	t * xz + s * axis.getY(),
				t * xy + s * axis.getZ(),	t * yy + c,					t * yz - s * axis.getX(),
				t * xz - s * axis.getY(),	t * yz + s * axis.getX(),	t * zz + c
			);
		}
		...
		_Matrix3x3 & setRotation(const vec3_t & dir,const vec3_t & up) {
			...
			set(
				baseX.getX(),baseY.getX(),baseZ.getX(),
				baseX.getY(),baseY.getY(),baseZ.getY(),
				baseX.getZ(),baseY.getZ(),baseZ.getZ());
			return *this;
		}
	//@}
	...
	//@{
		_Matrix3x3 getTransposed() const {
			return _Matrix3x3(at(R0C0), at(R1C0), at(R2C0),
							  at(R0C1), at(R1C1), at(R2C1),
							  at(R0C2), at(R1C2), at(R2C2));
		}
		_Matrix3x3 getInverse() const {
			...
			return _Matrix3x3(
				 (at(R1C1) * at(R2C2) - at(R1C2) * at(R2C1)) * dInv,
				-(at(R0C1) * at(R2C2) - at(R2C1) * at(R0C2)) * dInv,
				 (at(R0C1) * at(R1C2) - at(R1C1) * at(R0C2)) * dInv,

				-(at(R1C0) * at(R2C2) - at(R1C2) * at(R2C0)) * dInv,
				 (at(R0C0) * at(R2C2) - at(R2C0) * at(R0C2)) * dInv,
				-(at(R0C0) * at(R1C2) - at(R1C0) * at(R0C2)) * dInv,

				 (at(R1C0) * at(R2C1) - at(R2C0) * at(R1C1)) * dInv,
				-(at(R0C0) * at(R2C1) - at(R2C0) * at(R0C1)) * dInv,
				 (at(R0C0) * at(R1C1) - at(R0C1) * at(R1C0)) * dInv
			);
		}
		const _Matrix3x3 operator+(const _Matrix3x3 & b)const {
			return _Matrix3x3(at(R0C0) + b.at(R0C0), at(R0C1) + b.at(R0C1), at(R0C2) + b.at(R0C2),
							  at(R1C0) + b.at(R1C0), at(R1C1) + b.at(R1C1), at(R1C2) + b.at(R1C2),
							  at(R2C0) + b.at(R2C0), at(R2C1) + b.at(R2C1), at(R2C2) + b.at(R2C2));
		}
		const _Matrix3x3 operator-(const _Matrix3x3 & b)const {
			return _Matrix3x3(at(R0C0) - b.at(R0C0), at(R0C1) - b.at(R0C1), at(R0C2) - b.at(R0C2),
							  at(R1C0) - b.at(R1C0), at(R1C1) - b.at(R1C1), at(R1C2) - b.at(R1C2),
		...
		const _Matrix3x3 operator*(const _Matrix3x3 & b) const {
			return _Matrix3x3(
				at(R0C0) * b.at(R0C0) + at(R0C1) * b.at(R1C0) + at(R0C2) * b.at(R2C0),
		...
		const _Matrix3x3 operator*(value_t f) const {
			return _Matrix3x3(at(R0C0) * f, at(R0C1) * f, at(R0C2) * f,
							  at(R1C0) * f, at(R1C1) * f, at(R1C2) * f,
	...
	//@{
		...
		inline bool equals(const _Matrix3x3 & other, value_t epsilon) const {
			return  std::abs(at(R0C0)-other.at(R0C0))<=epsilon && std::abs(at(R0C1)-other.at(R0C1))<=epsilon && std::abs(at(R0C2)-other.at(R0C2))<=epsilon &&
					std::abs(at(R1C0)-other.at(R1C0))<=epsilon && std::abs(at(R1C1)-other.at(R1C1))<=epsilon && std::abs(at(R1C2)-other.at(R1C2))<=epsilon &&
					std::abs(at(R2C0)-other.at(R2C0))<=epsilon && std::abs(at(R2C1)-other.at(R2C1))<=epsilon && std::abs(at(R2C2)-other.at(R2C2))<=epsilon ;
		}
		inline bool operator==(const _Matrix3x3 & m) const {
			return  at(R0C0)==m.at(R0C0) && at(R0C1)==m.at(R0C1) && at(R0C2)==m.at(R0C2) &&
					at(R1C0)==m.at(R1C0) && at(R1C1)==m.at(R1C1) && at(R1C2)==m.at(R1C2) &&
					at(R2C0)==m.at(R2C0) && at(R2C1)==m.at(R2C1) && at(R2C2)==m.at(R2C2) ;
		}
		inline bool operator!=(const _Matrix3x3 & m) const {
			return  at(R0C0)!=m.at(R0C0) || at(R0C1)!=m.at(R0C1) || at(R0C2)!=m.at(R0C2) ||
					at(R1C0)!=m.at(R1C0) || at(R1C1)!=m.at(R1C1) || at(R1C2)!=m.at(R1C2) ||
	...
	//@{
		friend std::ostream & operator<<(std::ostream & out, const _Matrix3x3 & matrix) {
			return out	<< matrix.at(R0C0) << ' ' << matrix.at(R0C1) << ' ' << matrix.at(R0C2) << ' '
						<< matrix.at(R1C0) << ' ' << matrix.at(R1C1) << ' ' << matrix.at(R1C2) << ' '
						<< matrix.at(R2C0) << ' ' << matrix.at(R2C1) << ' ' << matrix.at(R2C2);
		}
		friend std::istream & operator>>(std::istream & in, _Matrix3x3 & matrix) {
			return in	>> matrix.at(R0C0) >> matrix.at(R0C1) >> matrix.at(R0C2)
						>> matrix.at(R1C0) >> matrix.at(R1C1) >> matrix.at(R1C2)
						>> matrix.at(R2C0) >> matrix.at(R2C1) >> matrix.at(R2C2);
		}
	//@}
