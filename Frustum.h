...
class Frustum {
	public:
		...
		const Vec3 & getPos()const						{	return orientation.getTranslation();	}
		Vec3 getDir()const								{	return orientation.getDirVector();		}
		Vec3 getUp()const								{	return orientation.getUpVector();		}
		float getNear()const							{	return nearValue;		}
		float getFar()const								{	return farValue;	}
		float getLeft()const							{	return leftValue;	}
		float getRight()const							{	return rightValue;	}
		float getBottom()const							{	return bottomValue;	}
		float getTop()const								{	return topValue;	}
		...
		void setFrustumFromAngles(const Angle & fovLeft,const Angle & fovRight,const Angle & fovBottom,const Angle & fovTop,
									float near,float far);
		void setOrthogonal(float l,float r,float b,float t,float n,float f)	{	setFrustum(l,r,b,t,n,f,true);	}
...
inline bool Frustum::pointInFrustum(const Vec3 &p)const {
	return 	planes[0].planeTest(p)>=0 &&
			planes[1].planeTest(p)>=0 &&
			planes[2].planeTest(p)>=0 &&
			planes[3].planeTest(p)>=0 &&
			planes[4].planeTest(p)>=0 &&
			planes[5].planeTest(p)>=0;
}
