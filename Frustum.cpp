...
void Frustum::setFrustum(float _left,float _right,float _bottom,float _top,float _near,float _far,bool _orthogonal /*= false*/){
	...
	// update matrix
	if(orthogonal){
		projectionMatrix = Matrix4x4::orthographicProjection(leftValue, rightValue, bottomValue, topValue, nearValue, farValue);
	}else{
		projectionMatrix = Matrix4x4::perspectiveProjection(leftValue, rightValue, bottomValue, topValue, nearValue, farValue);
	}
...
void Frustum::setFrustumFromAngles(const Angle & fovLeft,const Angle & fovRight,const Angle & fovBottom,const Angle & fovTop, float _near, float _far) {
	setFrustum( _near * std::tan(fovLeft.rad()), _near * std::tan(fovRight.rad()),
			_near * std::tan(fovBottom.rad()), _near * std::tan(fovTop.rad()), _near, _far,false);
}
...
void Frustum::recalculateCornersAndPlanes() {
	...
	// Careful: Z values are inverted by matrix. Therefore the front and back side are swapped (see sign of Z values below).
