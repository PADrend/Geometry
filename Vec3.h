...
class _Vec3 {
	...
	//@{
		...
		_Vec3() 						{	setValue(static_cast<value_t>(0));	}
		...
		_Vec3(value_t _x, value_t _y, value_t _z) 		{	setValue(_x,_y,_z);	}

		/*!	[ctor]
			@param v used to initialize the new Vec3
			@note values are converted by casting	*/
		template<class other_t>
		explicit _Vec3(const _Vec3<other_t> & v) 	{	setValue(static_cast<value_t>(v.x()), static_cast<value_t>(v.y()), static_cast<value_t>(v.z()));	}
		...
		explicit _Vec3(const value_t * v) 	{	setValue(v[0],v[1],v[2]);	}

		/*!	[ctor]
			@param v used for initialisation of all components	*/
		explicit _Vec3(const value_t v) 	{	setValue(v);	}
	//@}
	...
	//@{
		...
		inline const value_t getX() const 						{	return vec[0];	}
		inline const value_t x() const 							{	return vec[0];	}

		/*! @return the i-th component of this	*/
		inline const value_t get(const int & i) const 			{	return vec[i];	}

		/*! @return the y component of this
			@note equal to this[1]	*/
		inline const value_t getY() const 						{	return vec[1];	}
		inline const value_t y() const 							{	return vec[1];	}

		/*! @return the z component of this
			@note equal to this[2]	*/
		inline const value_t getZ() const 						{	return vec[2];	}
		inline const value_t z() const 							{	return vec[2];	}

		/*! @return the maximum absolute value of the components of this	*/
		inline value_t maxAbsValue()const {
			value_t maxi = std::abs(vec[0]);
			if(vec[1]>maxi)maxi = vec[1];
			else if(-vec[1]>maxi)maxi = -vec[1];
			if(vec[2]>maxi)maxi = vec[2];
			else if(-vec[2]>maxi)maxi = -vec[2];
			return maxi;
		}
	...
	//@{
		...
		inline void setValue(const value_t t) 	{	setValue(t,t,t);	}

		/*! sets the components of this
			@param v the first three values of v are used to set the components of this	*/
		inline void setValue(const value_t *v) 	{	setValue(v[0],v[1],v[2]);	}

		/*! sets the x component
			@param x the value the x component of this is set to
			@note equal to this[0]=x	*/
		inline void setX(const value_t _x)  		{	vec[0]=_x;	}
		inline void x(const value_t v) 			{	vec[0]=v;	}

		/*! sets the y component
			@param y the value the y component of this is set to
			@note equal to this[1]=y	*/
		inline void setY(const value_t _y)  		{	vec[1]=_y;	}
		inline void y(const value_t v) 			{	vec[1]=v;	}

		/*! sets the z component
			@param z the value the z component of this is set to
			@note equal to this[2]=z	*/
		inline void setZ(const value_t _z) 		{	vec[2]=_z;	}
		inline void z(const value_t v) 			{	vec[2]=v;	}
	...
	//@{
		...
		inline vec3_t cross(const vec3_t &p) const {
			return vec3_t(vec[1] * p.vec[2] - vec[2] * p.vec[1],
							 vec[2] * p.vec[0] - vec[0] * p.vec[2],
	...
	//@{
		...
		static inline vec3_t pairwiseMin(const vec3_t &a,const vec3_t &b) {
			return vec3_t(
					a.vec[0] < b.vec[0] ? a.vec[0] : b.vec[0],
					a.vec[1] < b.vec[1] ? a.vec[1] : b.vec[1],
