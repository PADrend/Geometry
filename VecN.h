...
class _VecN {
	...
	private:
		...
		template<class T> struct GetAbs {
			T operator()(const T & x) const {
				return std::abs(x);
			}
		};
		template<class T> struct GetMax {
			T operator()(const T & x, const T & y) const {
				return std::max(x, y);
			}
		};
		template<class T> struct GetMin {
			T operator()(const T & x, const T & y) const {
				return std::min(x, y);
			}
		};
		template<class T> struct GetAvg {
			T operator()(const T & x, const T & y) const {
				return (x + y) / 2;
			}
		};
	...
	public:
		 ...
		 * @return a new VecN
		 * @note the call VecNf(3,0); does not work, because 0 can be interpreted as nullptr pointer so the call gets ambiguous.
		 *          in this case remove the second parameter (no change as 0 is the default value) or add an explicit cast to value_t.
		 */
		...
		_VecN(_InputIterator _first, _InputIterator _end) : vec(_first, _end) {
		}
		 ...
		 */
		_VecN & operator+=(const value_t & constant) {
			std::transform(vec.begin(), vec.end(), vec.begin(), std::bind(std::plus<value_t>(), std::placeholders::_1, constant));
			return *this;
		}
		 ...
		 */
		_VecN & operator-=(const value_t & constant) {
			std::transform(vec.begin(), vec.end(), vec.begin(), std::bind(std::minus<value_t>(), std::placeholders::_1, constant));
			return *this;
		}
		...
		_VecN & operator*=(const float_t & constant) {
			std::transform(vec.begin(), vec.end(), vec.begin(), std::bind(std::multiplies<value_t>(), std::placeholders::_1, constant));
			return *this;
		}
		 ...
		 */
		_VecN & operator/=(const float_t & constant) {
			std::transform(vec.begin(), vec.end(), vec.begin(), std::bind(std::divides<value_t>(), std::placeholders::_1, constant));
			return *this;
		}
		...
		float_t length(uint32_t norm) const {
			...
			switch(norm) {
				...
				default: // >= 3
					...
					} else { // exponent is odd --> std::abs on base IS necessary (this works for all cases except maximum norm)
						for(const auto & it : vec) {
