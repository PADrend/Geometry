...
class _Box {
	...
	public:
		...
		_Box(const vec3_t & cornerA, const vec3_t & cornerB) :
			min(vec3_t::pairwiseMin(cornerA, cornerB)),
			max(vec3_t::pairwiseMax(cornerA, cornerB)) {
		}
	...
	private:
		...
		//@{
	public:
		value_t getMaxX() const		{	return max.getX();	}
		value_t getMaxY() const		{	return max.getY();	}
		value_t getMaxZ() const		{	return max.getZ();	}
		inline value_t getMax(dimension_t dim) const;
		const vec3_t & getMax() const {   return max;         }
		value_t getMinX() const		{	return min.getX();	}
		value_t getMinY() const		{	return min.getY();	}
		value_t getMinZ() const		{	return min.getZ();	}
		inline value_t getMin(dimension_t dim) const;
		const vec3_t & getMin() const {   return min;         }
		inline value_t getExtentMax() const;
		inline value_t getExtentMin() const;
		value_t getExtentX() const	{	return max.getX() - min.getX();	}
		value_t getExtentY() const	{	return max.getY() - min.getY();	}
		value_t getExtentZ() const	{	return max.getZ() - min.getZ();	}
		 ...
		 */
		static corner_t getOppositeCorner(const corner_t corner) {	return static_cast<corner_t> (corner ^ 7);	}
		/*
		...
		void setMaxX(value_t x)	{	max.setX(x);	}
		void setMaxY(value_t y)	{	max.setY(y);	}
		void setMaxZ(value_t z)	{	max.setZ(z);	}
		inline void setMax(dimension_t dim, value_t value);
		void setMax(const vec3_t & newMax) {
			max = newMax;
		}
		void setMinX(value_t x)	{	min.setX(x);	}
		void setMinY(value_t y)	{	min.setY(y);	}
		void setMinZ(value_t z)	{	min.setZ(z);	}
...
inline value_t _Box<value_t>::getSurfaceArea() const {
	return 2.0 * getExtentX() * getExtentY() +
		   2.0 * getExtentY() * getExtentZ() +
		   2.0 * getExtentX() * getExtentZ();
}

template<typename value_t>
inline _Vec3<value_t> _Box<value_t>::getCorner(corner_t nr) const {
	return vec3_t((nr & 1) ? max.getX() : min.getX(), (nr & 2) ? max.getY() : min.getY(), (nr & 4) ? max.getZ() : min.getZ());
}
...
inline corner_t _Box<value_t>::getOctant(const vec3_t &v)const{
	...
	if(v.getX()>center.getX()) index |= (1<<0);
	if(v.getY()>center.getY()) index |= (1<<1);
	if(v.getZ()>center.getZ()) index |= (1<<2);
...
inline bool _Box<value_t>::contains(const _Box &b) const {
	return min.getX()<=b.min.getX() && max.getX()>=b.max.getX() && min.getY()<=b.min.getY() && max.getY()>=b.max.getY() && min.getZ()<=b.min.getZ() && max.getZ()>=b.max.getZ();
}
