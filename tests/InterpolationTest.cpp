...

	{
		...
		for(int i = -10; i <= 20; ++i) {
			...
			if(t < 0.0) {
				...
				CPPUNIT_ASSERT_DOUBLES_EQUAL(w * w * p0 
											 + 2.0 * w * v * p1 
											 + v * v * p2, quadraticBezier(p0, p1, p2, t), epsilon);
			}
		}
	}
}
...

	{
		...
		for(int i = -10; i <= 20; ++i) {
			...
			if(t < 0.0) {
				...
				const double v = clamp(0.0, t, 1.0);
				const double w = 1.0 - v;
				CPPUNIT_ASSERT_DOUBLES_EQUAL(w * w * w * p0 
											 + 3.0 * w * w * v * p1 
											 + 3.0 * w * v * v * p2
											 + v * v * v * p3, cubicBezier(p0, p1, p2, p3, t), epsilon);
			}
		}
	}
}
