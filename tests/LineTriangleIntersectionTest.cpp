...
void LineTriangleIntersectionTest::testLineTriangleIntersection() {
	...
	const triangle_t triangle(vec_t(0.0, 0.0, 0.0),
							  vec_t(0.0, 1.0, 0.0),
							  vec_t(0.0, 0.0, 1.0));
	...
	for(double x = -3.0; x <= 3.0; x += 0.1) {
		for(double y = -3.0; y <= 3.0; y += 0.1) {
			for(double z = -3.0; z <= 3.0; z += 0.1) {
				...
				CPPUNIT_ASSERT_EQUAL(intersection, Geometry::Intersection::getLineTriangleIntersection(line, triangle, t, u, v));
				if(intersection) {
	 ...
	 * o---> x
	 */
	const triangle_t triangle2(vec_t(17, 5, 8),
							   vec_t(42, 6, 8),
							   vec_t(29, 19, 8));
	...
	CPPUNIT_ASSERT_EQUAL(false, Geometry::Intersection::getLineTriangleIntersection(line_t(t2, dirZ), triangle2, t, u, v));
	CPPUNIT_ASSERT_EQUAL(false, Geometry::Intersection::getLineTriangleIntersection(line_t(t3, dirZ), triangle2, t, u, v));
	CPPUNIT_ASSERT_EQUAL(false, Geometry::Intersection::getLineTriangleIntersection(line_t(t4, dirZ), triangle2, t, u, v));
}
