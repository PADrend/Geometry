...
void TriangleTest::testBarycentricCoordinates() {
	...
	CPPUNIT_ASSERT_EQUAL(Geometry::Vec3d(0.5, 0.5, 0.0), triangle.calcBarycentricCoordinates(Geometry::Vec3d(15.0, 0.0, 0.0)));
	CPPUNIT_ASSERT_EQUAL(Geometry::Vec3d(0.0, 0.5, 0.5), triangle.calcBarycentricCoordinates(Geometry::Vec3d(15.0, 15.0, 0.0)));
	CPPUNIT_ASSERT_EQUAL(Geometry::Vec3d(0.5, 0.0, 0.5), triangle.calcBarycentricCoordinates(Geometry::Vec3d(0.0, 15.0, 0.0)));

	// Check the centroid.
	CPPUNIT_ASSERT_EQUAL(Geometry::Vec3d(1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0), triangle.calcBarycentricCoordinates(Geometry::Vec3d(10.0, 10.0, 0.0)));
	
	CPPUNIT_ASSERT_EQUAL(Geometry::Vec3d(0.8, 0.1, 0.1), triangle.calcBarycentricCoordinates(Geometry::Vec3d(3.0, 3.0, 0.0)));
}
...
void TriangleTest::testTriangleArea() {
	...
	for(int i = 0 ; i<1000; ++i ){
		Geometry::Matrix3x3d r = Geometry::Matrix3x3d::createRotation(Geometry::Angle::rad(static_cast<float>(i)*0.01),Geometry::Vec3d(1.0,0.0,0.0));
		r.normOrthoLize();
		r = Geometry::Matrix3x3d::createRotation(Geometry::Angle::rad(static_cast<float>(i)*0.017),Geometry::Vec3d(0.0,1.0,0.0)) * r;
		r.normOrthoLize();
		r = Geometry::Matrix3x3d::createRotation(Geometry::Angle::rad(static_cast<float>(i)*0.027),Geometry::Vec3d(0.0,0.0,1.0)) * r;
		r.normOrthoLize();
