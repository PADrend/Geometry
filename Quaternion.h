...
class Quaternion {
	public:
		...
		const Quaternion operator*(value_t factor) const {
			return Quaternion(values[0] * factor,
							  values[1] * factor,
							  values[2] * factor,
							  values[3] * factor);
		}
		...
		const Quaternion operator+(const Quaternion & source) const {
			return Quaternion(values[0] + source[0],
							  values[1] + source[1],
							  values[2] + source[2],
							  values[3] + source[3]);
		}
		...
		static Quaternion eulerToQuaternion(const vec3_t & euler) {
			...
			return Quaternion(c1c2 * s3 + s1s2 * c3, s1 * c2 * c3 + c1 * s2 * s3, c1 * s2 * c3 - s1 * c2 * s3, c1c2 * c3 - s1s2 * s3);
		}

		vec3_t rotatePoint(const vec3_t & p1) {
			vec3_t p2;
			p2.setX(w() * w() * p1.x() + 2 * y() * w() * p1.z() - 2 * z() * w() * p1.y() + x() * x() * p1.x() + 2 * y() * x() * p1.y() + 2 * z() * x() * p1.z() - z() * z() * p1.x() - y() * y() * p1.x());
			p2.setY(2 * x() * y() * p1.x() + y() * y() * p1.y() + 2 * z() * y() * p1.z() + 2 * w() * z() * p1.x() - z() * z() * p1.y() + w() * w() * p1.y() - 2 * x() * w() * p1.z() - x() * x() * p1.y());
			p2.setZ(2 * x() * z() * p1.x() + 2 * y() * z() * p1.y() + z() * z() * p1.z() - 2 * w() * y() * p1.x() - y() * y() * p1.z() + 2 * w() * x() * p1.y() - x() * x() * p1.z() + w() * w() * p1.z());
			return p2;
		}
