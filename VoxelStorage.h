...
class VoxelStorage{
	...
	public:
		...
		typedef std::pair<
					std::vector<std::tuple<Vec3_t,uinteger_t,Voxel_t>>,	// uniform areas
	...
	private:
		...
		struct Area{
			...
			Area(const Vec3_t & _origin, uinteger_t _sideLength, const Voxel_t &_uniformValue) : 
					origin(_origin), sideLength(_sideLength), dataType(DataType::UNIFORM_AREA), uniformValue(_uniformValue){
			}
			~Area(){	clear();	}
			void clear(){
				if(isContainer()){
			...
			uint8_t getChildIndex(const Vec3_t & pos)const{
				...
				return	(pos.x()>=origin.x()+hSideLength ? 1 : 0) +
						(pos.y()>=origin.y()+hSideLength ? 2 : 0) +
			...
			Vec3_t getOctantOrigin(uint8_t i)const{
				const auto hSideLength = static_cast<integer_t>(sideLength) / 2;
				return Vec3_t(	origin.x()+((i&1)>0 ? hSideLength : 0),
								origin.y()+((i&2)>0 ? hSideLength : 0),
								origin.z()+((i&4)>0 ? hSideLength : 0));
			}
			bool isContainer()const			{	return dataType == DataType::CONTAINER;	}
			bool isBlock()const				{	return dataType == DataType::BLOCK;	}
			bool isUniform()const			{	return dataType == DataType::UNIFORM_AREA;	}
			
			const Vec3_t& getOrigin()const	{	return origin;	}
			Vec3_t getMaxPosition()const	{	return Vec3_t(origin.x()+sideLength-1,origin.y()+sideLength-1,origin.z()+sideLength-1);	}
			block_t* getBlock()const		{	return isBlock() ? data.block : nullptr;	}
			Area* getChild(uint8_t i)const	{	return isContainer() ? data.children->at(i) : nullptr;	}

			Box_t getBox()const				{	return Box_t(origin, origin+Vec3_t(sideLength-1,sideLength-1,sideLength-1)); }
		...
		uint32_t posToBlockIdx(const Vec3_t& pos)const{
			return	(pos.x()&blockMask) + 
					(pos.y()&blockMask)*blockSideLength + 
...
//			std::cout << "b: "<<newOrigin<<" : "<<newSideLength<<std::endl;
			while( !Box_t(newOrigin, newOrigin+Vec3_t(newSideLength-1,newSideLength-1,newSideLength-1)).contains(pos) ){
				newSideLength*=2;
...
//				std::cout << " >("<<currentArea->origin<<" : "<<currentArea->sideLength<< ")"<<std::endl;
				...
				if(currentArea->isBlock()){
					...
					if(!child){
						currentArea = currentArea->insertIntermediateChild(calcOrigin(pos,blockSideLength),blockSideLength);
					}else if(child->contains(pos)){
						currentArea = child;
					}else{ // insert intermediate area
						
//						std::cout << "B) pos: "<<pos<< " box:"<< currentArea->origin<<":"<<currentArea->sideLength <<" ";
				
						const auto commonBox =  getEnclosingAreaBox( *child,pos );
//						std::cout << "child:"<< child->origin<<":"<<child->sideLength <<" ";
//						std::cout << "commonBox:"<< commonBox.first<<":"<<commonBox.second <<" "<<std::endl;
...
//					std::cout << " >F "<< currentArea->origin<<" : "<<currentArea->sideLength <<std::endl;
...
//						std::cout << " >B "<< currentArea->origin<<" : "<<currentArea->sideLength <<std::endl;
					}else{ // uniform value or container
						currentArea->assureContainer();
						for(uint8_t i=0;i<8;++i){
							const Box_t octantIntersection = Intersection::getBoxBoxIntersection(currentArea->getOctant(i),fillArea);
...
//								std::cout << " >>a:              \t"<< currentArea->getOrigin() <<std::endl;
//								std::cout << " >>octant:"<<static_cast<int>(i)<<"         \t"<< currentArea->getOctant(i) <<std::endl;
//								std::cout << " >>octantIntersection:\t"<< octantIntersection <<std::endl;
//								std::cout << " >>enclosingBox:\t"<< enclosingBox.first<<" : "<<enclosingBox.second <<std::endl;
...
//								std::cout << "\n >> combined:\t"<<b;
//								std::cout << "\n >> area:\t"<<enclosingBox.first<<" : "<<enclosingBox.second<<"\n";
								todo.push( currentArea->insertIntermediateChild(enclosingBox.first,enclosingBox.second) );
							}
						}
					}
			...
			while(!todo.empty()){
				...
				if(currentArea->isBlock()){
					...
					if(currentArea->uniformValue!=nullVoxel ) // skip null
						uniformAreas.emplace_back( currentArea->getOrigin(), currentArea->sideLength, currentArea->uniformValue );
				}
			}
		...
		void deserialize(const serializationData_t & data){
			for(const auto& uniformArea: data.first){
				const Box_t b( std::get<0>(uniformArea), 
								std::get<0>(uniformArea)+Vec3_t(std::get<1>(uniformArea),std::get<1>(uniformArea),std::get<1>(uniformArea)));
