...
class _Matrix4x4 {
	...
	private:
		...
		//@{
	public:
		_Matrix4x4() 							{	setIdentity();	}

		_Matrix4x4(
				value_t d0,value_t d1,value_t d2,value_t d3,
				value_t d4,value_t d5,value_t d6,value_t d7,
				value_t d8,value_t d9,value_t d10,value_t d11,
				value_t d12,value_t d13,value_t d14,value_t d15) {
			m[0]=d0;    m[1]=d1;    m[2]=d2;    m[3]=d3;
			m[4]=d4;    m[5]=d5;    m[6]=d6;    m[7]=d7;
			m[8]=d8;    m[9]=d9;    m[10]=d10;  m[11]=d11;
			m[12]=d12;  m[13]=d13;  m[14]=d14;  m[15]=d15;
		}

		explicit _Matrix4x4(const value_t * data) 	{	std::copy(data, data + 16, m);	}

		explicit _Matrix4x4(const srt_t & srt)	{	srt.toArray(m);	}

		//! Convert a 3 times 3 matrix to a 4 times 4 matrix by adding a one in the last entry, and zeros in the fourth column and fourth row.
		 ...
		 */
		value_t& operator[](const unsigned int i) {	return m[i];	}
		/**
		 *   @param i an index
				  *   @return the i-th element of this matrix
		 *   @note elements are arranged row-wise, so i=3 will result in the 1st column in the 2nd row
		 */
		value_t at(const unsigned int i)const 	{	return m[i];	}
		 ...
		 */
		const value_t* getData()const 			{	return m;	}
		/**
		*   @return the determinant of this
		*/
		value_t det()const {
			return   m[ 0] * m[ 5] * m[10] * m[15] - m[ 0] * m[ 5] * m[11] * m[14] - m[ 0] * m[ 9] * m[ 6] * m[15] + m[ 0] * m[ 9] * m[ 7] * m[14]
					 + m[ 0] * m[13] * m[ 6] * m[11] - m[ 0] * m[13] * m[ 7] * m[10] - m[ 4] * m[ 1] * m[10] * m[15] + m[ 4] * m[ 1] * m[11] * m[14]
					 + m[ 4] * m[ 9] * m[ 2] * m[15] - m[ 4] * m[ 9] * m[ 3] * m[14] - m[ 4] * m[13] * m[ 2] * m[11] + m[ 4] * m[13] * m[ 3] * m[10]
					 + m[ 8] * m[ 1] * m[ 6] * m[15] - m[ 8] * m[ 1] * m[ 7] * m[14] - m[ 8] * m[ 5] * m[ 2] * m[15] + m[ 8] * m[ 5] * m[ 3] * m[14]
					 + m[ 8] * m[13] * m[ 2] * m[ 7] - m[ 8] * m[13] * m[ 3] * m[ 6] - m[12] * m[ 1] * m[ 6] * m[11] + m[12] * m[ 1] * m[ 7] * m[10]
					 + m[12] * m[ 5] * m[ 2] * m[11] - m[12] * m[ 5] * m[ 3] * m[10] - m[12] * m[ 9] * m[ 2] * m[ 7] + m[12] * m[ 9] * m[ 3] * m[ 6];
		}
		...
		//@{
		void setIdentity() {
			m[1]=m[2]=m[3]=m[4]=m[6]=m[7]=m[8]=m[9]=m[11]=m[12]=m[13]=m[14]=static_cast<value_t>(0.0);
		...
		*/
		...
		static _Matrix4x4 perspectiveProjection(value_t l,value_t r, value_t b,value_t t,value_t n,value_t f) {
			...
			return _Matrix4x4(	twoN / (r - l),	0,				(r + l) / (r - l),	0,
								0,				twoN / (t - b),	(t + b) / (t - b),	0,
								0,				0,				-(f + n) / (f - n),	-(twoN * f) / (f - n),
								0,				0,				minusOne,			0);
		}
		/**
		...
		static _Matrix4x4 orthographicProjection(value_t l, value_t r, value_t b, value_t t, value_t n, value_t f) {
			...
			return _Matrix4x4(	two / (r - l),	0,				0,					-(r + l) / (r - l),
								0,				two / (t - b),	0,					-(t + b) / (t - b),
								0,				0,				-two / (f - n),		-(f + n) / (f - n),
								0,				0,				0,					one);
		}
		...
		const vec3_t transformDirection(value_t x, value_t y, value_t z) const {
			 ...
			 */
			return vec3_t(m[0] * x + m[1] * y +  m[2] * z,
						  m[4] * x + m[5] * y +  m[6] * z,
						  m[8] * x + m[9] * y + m[10] * z);
		}
		...
		const vec3_t transformPosition(value_t x, value_t y, value_t z) const {
			...
			return vec3_t((m[0] * x + m[1] * y +  m[2] * z +  m[3]) * wInv,
						  (m[4] * x + m[5] * y +  m[6] * z +  m[7]) * wInv,
		...
		_Matrix4x4<value_t> inverse()const {
			...
			ret.m[ 0] = (  m[ 5] * m[10] * m[15] - m[ 5] * m[11] * m[14] - m[ 9] * m[ 6] * m[15] + m[ 9] * m[ 7] * m[14] + m[13] * m[ 6] * m[11] - m[13] * m[ 7] * m[10]) *  d;
			ret.m[ 1] = (  m[ 1] * m[10] * m[15] - m[ 1] * m[11] * m[14] - m[ 9] * m[ 2] * m[15] + m[ 9] * m[ 3] * m[14] + m[13] * m[ 2] * m[11] - m[13] * m[ 3] * m[10]) * -d;
			ret.m[ 2] = (- m[ 1] * m[ 6] * m[15] + m[ 1] * m[ 7] * m[14] + m[ 5] * m[ 2] * m[15] - m[ 5] * m[ 3] * m[14] - m[13] * m[ 2] * m[ 7] + m[13] * m[ 3] * m[ 6]) * -d;
			ret.m[ 3] = (  m[ 1] * m[ 6] * m[11] - m[ 1] * m[ 7] * m[10] - m[ 5] * m[ 2] * m[11] + m[ 5] * m[ 3] * m[10] + m[ 9] * m[ 2] * m[ 7] - m[ 9] * m[ 3] * m[ 6]) * -d;

			ret.m[ 4] = (  m[ 4] * m[10] * m[15] - m[ 4] * m[11] * m[14] - m[ 8] * m[ 6] * m[15] + m[ 8] * m[ 7] * m[14] + m[12] * m[ 6] * m[11] - m[12] * m[ 7] * m[10]) * -d;
			ret.m[ 5] = (  m[ 0] * m[10] * m[15] - m[ 0] * m[11] * m[14] - m[ 8] * m[ 2] * m[15] + m[ 8] * m[ 3] * m[14] + m[12] * m[ 2] * m[11] - m[12] * m[ 3] * m[10]) *  d;
			ret.m[ 6] = (  m[ 0] * m[ 6] * m[15] - m[ 0] * m[ 7] * m[14] - m[ 4] * m[ 2] * m[15] + m[ 4] * m[ 3] * m[14] + m[12] * m[ 2] * m[ 7] - m[12] * m[ 3] * m[ 6]) * -d;
			ret.m[ 7] = ( -m[ 0] * m[ 6] * m[11] + m[ 0] * m[ 7] * m[10] + m[ 4] * m[ 2] * m[11] - m[ 4] * m[ 3] * m[10] - m[ 8] * m[ 2] * m[ 7] + m[ 8] * m[ 3] * m[ 6]) * -d;

			ret.m[ 8] = ( -m[ 4] * m[ 9] * m[15] + m[ 4] * m[11] * m[13] + m[ 8] * m[ 5] * m[15] - m[ 8] * m[ 7] * m[13] - m[12] * m[ 5] * m[11] + m[12] * m[ 7] * m[ 9]) * -d;
			ret.m[ 9] = (  m[ 0] * m[ 9] * m[15] - m[ 0] * m[11] * m[13] - m[ 8] * m[ 1] * m[15] + m[ 8] * m[ 3] * m[13] + m[12] * m[ 1] * m[11] - m[12] * m[ 3] * m[ 9]) * -d;
			ret.m[10] = (  m[ 0] * m[ 5] * m[15] - m[ 0] * m[ 7] * m[13] - m[ 4] * m[ 1] * m[15] + m[ 4] * m[ 3] * m[13] + m[12] * m[ 1] * m[ 7] - m[12] * m[ 3] * m[ 5]) *  d;
			ret.m[11] = (  m[ 0] * m[ 5] * m[11] - m[ 0] * m[ 7] * m[ 9] - m[ 4] * m[ 1] * m[11] + m[ 4] * m[ 3] * m[ 9] + m[ 8] * m[ 1] * m[ 7] - m[ 8] * m[ 3] * m[ 5]) * -d;

			ret.m[12] = (  m[ 4] * m[ 9] * m[14] - m[ 4] * m[10] * m[13] - m[ 8] * m[ 5] * m[14] + m[ 8] * m[ 6] * m[13] + m[12] * m[ 5] * m[10] - m[12] * m[ 6] * m[ 9]) * -d;
			ret.m[13] = ( -m[ 0] * m[ 9] * m[14] + m[ 0] * m[10] * m[13] + m[ 8] * m[ 1] * m[14] - m[ 8] * m[ 2] * m[13] - m[12] * m[ 1] * m[10] + m[12] * m[ 2] * m[ 9]) * -d;
			ret.m[14] = (  m[ 0] * m[ 5] * m[14] - m[ 0] * m[ 6] * m[13] - m[ 4] * m[ 1] * m[14] + m[ 4] * m[ 2] * m[13] + m[12] * m[ 1] * m[ 6] - m[12] * m[ 2] * m[ 5]) * -d;
			ret.m[15] = (  m[ 0] * m[ 5] * m[10] - m[ 0] * m[ 6] * m[ 9] - m[ 4] * m[ 1] * m[10] + m[ 4] * m[ 2] * m[ 9] + m[ 8] * m[ 1] * m[ 6] - m[ 8] * m[ 2] * m[ 5]) *  d;
	...
	public:
		...
		friend std::istream & operator>>(std::istream & in, _Matrix4x4<value_t> & matrix) {
			return in	>> matrix.m[ 0] >> matrix.m[ 1] >> matrix.m[ 2] >> matrix.m[ 3]
						>> matrix.m[ 4] >> matrix.m[ 5] >> matrix.m[ 6] >> matrix.m[ 7]
						>> matrix.m[ 8] >> matrix.m[ 9] >> matrix.m[10] >> matrix.m[11]
						>> matrix.m[12] >> matrix.m[13] >> matrix.m[14] >> matrix.m[15];
		}
