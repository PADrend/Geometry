...
class _Rect {
	public:
		 ...
		 */
		//@{
		vec2_t getSize() const				{	return vec2_t(width,height);	}
		value_t getX() const				{	return x;				}
		value_t getMinX() const				{	return x;				}
		value_t getMaxX() const				{	return x + width;		}
		value_t getY() const				{	return y;				}
		value_t getMinY() const				{	return y;				}
		value_t getMaxY() const				{	return y + height;		}
		value_t getWidth() const			{	return width;			}
		value_t getHeight() const			{	return height;			}
		value_t getArea() const				{	return width * height;	}
		vec2_t getPosition() const			{	return vec2_t(x, y);	}
		vec2_t getCenter() const {
			return vec2_t((getMinX() + getMaxX()) / static_cast<value_t>(2), (getMinY() + getMaxY()) / static_cast<value_t>(2));
		}
		 ...
		 */
		//@{
		value_t & _accessX()			{	return x;		}
		value_t & _accessY()			{	return y;		}
		value_t & _accessWidth()		{	return width;	}
		value_t & _accessHeight()	{	return height;	}
		//@}
		 ...
		 */
		//@{
		void setPosition(value_t _x, value_t _y)			{	x = _x;			y = _y;			}
		void setPosition(const vec2_t & vec)	{	x = vec.getX();	y = vec.getY();	}
		void setX(value_t _x)						{	x = _x;							}
		void setY(value_t _y)						{	y = _y;							}
		void setWidth(value_t w)						{	width = w;						}
		void setHeight(value_t h)					{	height = h;						}
		void setSize(value_t w, value_t h)				{	width = w;		height = h;		}
		void setSize(const vec2_t & s)			{	width = s.x();	height = s.y();	}
