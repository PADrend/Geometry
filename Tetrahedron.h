...
class Tetrahedron {
	...
	public:
		 ...
		 * @return pair(success as bool,Barycentric coordinates as vec4) success is false, iff the tetrahedron is degenerated.
		...
		bool containsPoint(const vec3_t & point, value_t epsilon) const {
			return	pA.planeTest(point) <= epsilon &&
					pB.planeTest(point) <= epsilon &&
					pC.planeTest(point) <= epsilon &&
