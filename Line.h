...
class _LineBase {
	...
	//@{
	public:
		value_t getMinParam() const 			{	return minParam;	}
		value_t getMaxParam() const 			{	return maxParam;	}
		const vec_t & getDirection() const		{	return direction;	}
		const vec_t & getOrigin() const 		{	return origin;	}
		...
		bool operator==(const impl_t & other) const {
			return origin==other.origin && direction==other.direction &&
					minParam==other.minParam && maxParam==other.maxParam;
		}
	//@}
	...
	protected: // \note these methods are proteced as they should not be available in all subclasses.
		 ...
		 */
		void _setDirection(const vec_t & newDirection) 	{	direction = newDirection;	}
		/**
		 * Specify a new base vector for the line.
		 *
		 * @param newOrigin Point vector
		 */
		void _setOrigin(const vec_t & newOrigin) 		{	origin = newOrigin;	}

		/**
		 * Normalize the direction vector.
		 * This should be done before any distance calculation!
		 */
		void _normalize()								{	direction.normalize();	}
	//@}
};

// -----------------------------------------------------------------------------------------------------------
template<typename vec_t> class _Ray; // forward declaration
...
class _Line : public _LineBase<vec_t,_Line > {
	...
	public:
		...
		_Line(const vec_t & _origin, const vec_t & _direction) :
				 base_t(_origin,_direction,-std::numeric_limits<value_t>::infinity(),std::numeric_limits<value_t>::infinity()) {
		}
		_Line() : base_t(vec_t(),vec_t(),-std::numeric_limits<value_t>::infinity(),std::numeric_limits<value_t>::infinity()){
		}

		/**
		 * Calculate a pair of points (= values for points) on this line and the given @a lineB that is closest to each other.
		 *
		 * @param lineB The second line
		 * @return pair(parameterA,parameterB) Return value that can be used to calculate the point on the lines
		 * @note The function @a _Line<_T>::getPoint can be used with the returned parameters as argument to calculate the points.
		...
		std::pair<value_t,value_t> getClosestValues(const line_t & lineB)const {
			 ...
			 * The Hessian matrix is H(f) = ( (δ²f/(δx δx), δ²f/(δx δy)), (δ²f/(δy δx), δ²f/(δy δy)) ) = ( (2 a, 2 b), (2 b, 2 c) ).
			...
			if (det >= std::numeric_limits<value_t>::epsilon()) {
				...
				return std::make_pair(
						(b * e - d) * invDet, // parameterA: drop c here because it is 1
		 ...
		 * @note The function @a _Line<_T>::getPoint and @a _Ray<_T>::getPoint can be used with the returned parameters as argument to calculate the points.
		 * @note The direction vectors is required to have unit length.
		 * @author Benjamin Eikel
		 * @date 2010-10-14
		 */
		...
		void normalize()								{	base_t::_normalize();	}
		void setDirection(const vec_t & newDirection) 	{	base_t::_setDirection(newDirection);	}
		void setOrigin(const vec_t & newOrigin) 		{	base_t::_setOrigin(newOrigin);	}
};
...
class _Ray : public _LineBase<vec_t,_Ray> {
	...
	public:
		...
		_Ray() :
			base_t(vec_t(), vec_t(), static_cast<value_t>(0), std::numeric_limits<value_t>::infinity()) {
		}
...
class _Segment : public _LineBase<vec_t,_Segment> {
	...
	public:
		_Segment(const vec_t& fromPoint, const vec_t & toPoint) :
				base_t(fromPoint, (toPoint - fromPoint), static_cast<value_t>(0), static_cast<value_t>((toPoint - fromPoint).length())) {
			if(base_t::getMaxParam() > 0.0) {
				base_t::_setDirection(base_t::getDirection() / base_t::getMaxParam());
			}
		}
		_Segment() :
			base_t(vec_t(), vec_t(), static_cast<value_t>(0), static_cast<value_t>(0)) {
		}
		value_t length()const					{	return base_t::getMaxParam();	}
