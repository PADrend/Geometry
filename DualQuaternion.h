...
namespace Geometry {
    ...
    public:
        ...
        _DualQuaternion(value_t _rx, value_t _ry, value_t _rz, value_t _rw,
                        value_t _tx, value_t _ty, value_t _tz, value_t _tw)
        {
        ...
        const _DualQuaternion normalizeTranslation() const {
            ...
            return _DualQuaternion(getRotation(), getTranslation() - getRotation().operator*((getRotation().dot(getTranslation()))*magn));
        }
        ...
        const _DualQuaternion operator*(const value_t factor) const {
            return _DualQuaternion (rot[0] * factor,
                                             rot[1] * factor,
                                             rot[2] * factor,
                                             rot[3] * factor,
                                             trans[0] * factor,
        ...
        const _DualQuaternion operator+(const _DualQuaternion & dq) const {
            return _DualQuaternion(rX() + dq.rX(),
                                            rY() + dq.rY(),
                                            rZ() + dq.rZ(),
                                            rW() + dq.rW(),
                                            tX() + dq.tX(),
        ...
        static _DualQuaternion dualQuaternionLinearInterpolation(const _DualQuaternion & dq1, const _DualQuaternion & dq2, const value_t factor, bool takeShortestArc=false) {
        ...
        static _DualQuaternion convertFromMatrix(const _Matrix4x4<value_t> & matrix)
        {
            ...
            _Matrix3x3<value_t> rotMat(matrix.at(0), matrix.at(1), matrix.at(2),
                                       matrix.at(4), matrix.at(5), matrix.at(6),
        ...
        {
            return out << dq.rX() << ' ' << dq.rY() << ' ' << dq.rZ() << ' ' << dq.rW() << ' '
            << dq.tX() << ' ' << dq.tY() << ' ' << dq.tZ() << ' ' << dq.tW();
        }
