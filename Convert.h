...
struct Convert {
	...
	template<typename float_t,
			 ...
			 typename = typename	std::enable_if<
										std::is_integral<signed_t>::value &&
										std::is_signed<signed_t>::value &&
	...
	static float_t fromSignedTo(signed_t c) {
		...
		const float_t f = c / 
			static_cast<float_t>(std::numeric_limits<signed_t>::max());
		return std::min(std::max(f, static_cast<float_t>(-1)),
	...
	template<typename float_t,
			 typename unsigned_t,
			 typename = typename	std::enable_if<
										std::is_integral<unsigned_t>::value &&
										std::is_unsigned<unsigned_t>::value &&
										std::is_floating_point<float_t>::value
									>::type>
	static float_t fromUnsignedTo(unsigned_t c) {
		// max() is equal to 2^b - 1 for unsigned types with b bits
		const float_t f = c / 
			static_cast<float_t>(std::numeric_limits<unsigned_t>::max());
	...
	static signed_t toSigned(float_t f) {
		const float_t fC = std::min(std::max(static_cast<float_t>(-1), f),
									static_cast<float_t>(1));
		// max() is equal to 2^(b-1) - 1 for signed types with b bits
	...
	static unsigned_t toUnsigned(float_t f) {
		const float_t fC = std::min(std::max(static_cast<float_t>(0), f),
									static_cast<float_t>(1));
		// max() is equal to 2^b - 1 for unsigned types with b bits
