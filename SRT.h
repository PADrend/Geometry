...
class _SRT {
	...
	//@{
		 ...
		 */
		_SRT() :
			s(static_cast<value_t>(1.0)), r(), t(), rotationCounter(0) {}
		/**
		...
		_SRT(const _SRT<value_t> & srt1, const _SRT<value_t> & srt2, float blend) :
			...
			r(srt1.r, srt2.r, blend), t(srt1.t, srt2.t, blend), rotationCounter(0) {
		}
	 ...
	 */
	//@{
		value_t getScale() const					{	return s;	}
		const vec3_t & getTranslation() const 	{	return t;	}
		const matrix3x3_t & getRotation() const 	{	return r;	}
		vec3_t getDirVector() const			{	return r.getCol(matrix3x3_t::FRONT); 	}
		vec3_t getRightVector() const		{	return r.getCol(matrix3x3_t::RIGHT);	}
		vec3_t getUpVector() const 			{	return r.getCol(matrix3x3_t::UP);	}
	//@}
