...
namespace Geometry {
template<typename vec_t> class _Line;
template<typename vec_t> class _Ray;
template<typename vec_t> class _Segment;
...
class _Plane {
	...
	public:
		...
		inline const vec3_t & getNormal()const					{	return normal;	}

		inline const value_t & getOffset()const					{	return offset;	}
		...
		inline bool isUndefined()const							{	return normal.isZero();	}

		inline bool operator==(const _Plane & other )const		{	return offset==other.offset && normal==other.normal;	}
		 ...
		 *          below plane < 0
		   *          above plane > 0
		 */
		inline float planeTest(const vec3_t & point)const 		{	return point.dot(normal)-offset;	}
	...
	//@{
		...
		inline void setNormal(const vec3_t & _normal)	{	normal = _normal;	}

		inline void setOffset(const value_t _offset)	{	offset = _offset;	}
	//@}
